...
from src.views.movie.comment_form import CommentForm

movie_blueprint = Blueprint("movies", __name__)

@movie_blueprint.route("/view_movie/<int:movie_id>", methods=["GET", "POST"])
def view_movie(movie_id):
    ...
    if form.validate_on_submit():
        ...
        Comment(author=current_user.username, text=form.text.data, movie_id=movie.id).create()
        return redirect(url_for("movies.view_movie", movie_id=movie_id))

    comments = Comment.query.filter_by(movie_id=movie_id).all()
    return render_template("movie/view_movie.html", movie=movie, form=form, comments=comments)
...
def create_movie():
    ...
    if form.validate_on_submit():
        ...
        new_movie = Movie(
            ...
            watch_links=form.watch_links.data,
            img=f"img/{filename}"
        )
        new_movie.create()
...
def edit_movie(movie_id):
    ...
    if form.validate_on_submit():
        ...
        movie.watch_links = form.watch_links.data
